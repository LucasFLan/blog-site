    ...
    <section className="container py-8 md:py-10">
      <div className="flex flex-col items-center gap-12">
        ...
        <div className="grid gap-8 md:grid-cols-2">
          {customerStories.map((cutomerStory) => (
            <div
              key={cutomerStory.author.name}
              className="flex flex-col gap-6 rounded-lg bg-gray-500 p-6 md:p-12"
            >
              <p className="text-balance text-gray-200 ">
                {cutomerStory.content}
              </p>
              <div className="flex items-center gap-3">
                <div className="relative h-10 w-10 overflow-hidden rounded-full">
                  <Image
                    src={cutomerStory.author.avatar}
                    alt={`Foto do cliente: ${cutomerStory.author.name}`}
                    fill
                ...
                <div className="flex flex-col">
                  <strong className="text-gray-200 text-sm">
                    {cutomerStory.author.name}
                  </strong>
                  <span className="text-xs text-gray-300">
                    {cutomerStory.author.role}
                  </span>
                </div>
              </div>
            </div>
